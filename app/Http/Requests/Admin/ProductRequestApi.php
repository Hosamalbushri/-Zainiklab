...
class ProductRequestApi extends FormRequest
{
    ...
    public function rules()
    {
        ...
        $thumbnailMaxKb = PRODUCT_THUMBNAIL_MAX_FILE_KB;
        $selectedFileType = strtolower((string) request()->get('file_types'));
        $fileExtensionRule = function ($attribute, $value, $fail) use ($selectedFileType) {
            if (!$value || !$selectedFileType || $selectedFileType === 'other') {
                return;
            }

            $uploadedExtension = strtolower((string) $value->getClientOriginalExtension());
            if ($uploadedExtension !== $selectedFileType) {
                $fail(__('This filed must be type of selected ').request()->get('file_types'));
            }
        };
        ...
        $rules = [
            ...
            'thumbnail_image' => 'bail|nullable|image|mimes:jpg,jpeg,png|max:'.$thumbnailMaxKb,
            'use_this_photo' => 'exclude_unless:accessibility,2|required',
            'main_file' => ['bail','exclude_unless:accessibility,2','required','exclude_if:file_types,Other','file',$fileExtensionRule],
        ...
        if(request()->get('accessibility') == 1){
            ...
            foreach(request()->get('variations') ?? [] as $index => $variation){
                ...
                $rules['main_files_'.$index] = ['bail',(!isset($variation_ids[$index]) || $variation_ids[$index] == "" ? 'required' : 'nullable' ),'exclude_if:file_types,Other','file',$fileExtensionRule];
        ...
        if($ifThumbnailRequired == ACTIVE){
            $rules['thumbnail_image'] = 'bail|required|image|mimes:jpeg,jpg,png,gif,tif,bmp,ico,psd,webp|max:'.$thumbnailMaxKb;
        }

        if(!is_null(request()->get('uuid'))){
            $rules['main_file'] = ['bail','exclude_unless:accessibility,2','nullable','exclude_if:file_types,Other','file',$fileExtensionRule];
            $rules['thumbnail_image'] = 'bail|nullable|image|mimes:jpeg,jpg,png,gif,tif,bmp,ico,psd,webp|max:'.$thumbnailMaxKb;
        }
    ...
    public function messages()
    {
        ...
        return [
            ...
            'thumbnail_image.max' => __('The thumbnail image may not be greater than 10 MB.'),
        ];
    }
