...
@section('content')
    ...
    <div class="p-sm-30 p-15">
        <div class="d-flex align-items-center justify-content-between flex-wrap g-15 pb-26">
            <h2 class="fs-24 fw-600 lh-29 text-primary-dark-text">{{__('Add Plan')}}</h2>
            <div class="">
                    ...
                    <div class="breadcrumb__content__right">
                        <nav aria-label="breadcrumb">
                            <ul class="breadcrumb sf-breadcrumb">
                                ...
                                <li class="breadcrumb-item"><a
                                        href="{{route('admin.plan.index')}}">{{__('All Plan')}}</a></li>
        ...
        <div class="bg-white bd-one bd-c-stroke bd-ra-10 p-sm-30 p-15">
            <form action="{{route('admin.plan.store')}}" method="post" class="form-horizontal" enctype="multipart/form-data">
                @csrf
                ...
                <div class="row rg-24 bd-b-one bd-c-stroke pb-24 mb-24">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="name" value="{{old('name')}}"
                               placeholder="{{__('Type Title')}}" class="zForm-control" required>
                        @if ($errors->has('name'))
                            <span class="text-danger"><i class="fas fa-exclamation-triangle"></i> {{ $errors->first('name') }}</span>
                        @endif
                    </div>
                    <div class="col-md-6">
                        <label class="zForm-label" for="subtitle">{{__('Subtitle')}} <span class="text-danger">*</span></label>
                        <input type="text" name="subtitle" value="{{old('subtitle')}}" placeholder="{{__('Type Subtitle')}}" class="zForm-control" required>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="description" value="{{old('description')}}" placeholder="{{__('Type Description')}}" class="zForm-control" required>
                        @if ($errors->has('description'))
                            <span class="text-danger"><i class="fas fa-exclamation-triangle"></i> {{ $errors->first('description') }}</span>
                        @endif
                    </div>
                    <div class="col-md-6">
                        <label class="zForm-label" for="device_limit">{{__('Number of Device Limit')}} <span class="text-danger">*</span></label>
                        <input type="number" min="1" name="device_limit" value="{{old('device_limit')}}" placeholder="{{__('Type device access limit')}}" class="zForm-control" required>
                        @if ($errors->has('device_limit'))
                            <span class="text-danger"><i class="fas fa-exclamation-triangle"></i> {{ $errors->first('device_limit') }}</span>
                        @endif
                    </div>
                    <div class="col-md-6">
                        <label class="zForm-label" for="name">{{__('Monthly Price')}} <span class="text-danger">*</span></label>
                        <div class="input-group flex-nowrap">
                            <input type="number" min="0" name="monthly_price" value="{{old('monthly_price')}}" placeholder="{{__('Type monthly price')}}" class="zForm-control" required>
                    ...
                    <div class="col-md-6">
                            <label class="zForm-label" for="name">{{__('Yearly Price')}} <span
                                    class="text-danger">*</span></label>
                            <div class="input-group flex-nowrap">
                                <input type="number" min="0" name="yearly_price"
                                       value="{{old('yearly_price')}}"
                                       placeholder="{{__('Type yearly price')}}" class="zForm-control"
                                       required>
                                <span class="input-group-text">{{ getCurrencySymbol() }}</span>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-control" name="tax_id" id="tax_id">
                            ...
                            @foreach($taxes as $tax)
                                <option
                                    value="{{ $tax->id }}" {{ (old('tax_id') == $tax->id) ? 'selected' : null }}>{{ $tax->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6">
                        <label class="zForm-label" for="download_limit_type">{{ __('Download Limit Type') }} <span
                                class="text-danger">*</span></label>
                        <select class="form-control" name="download_limit_type" id="download_limit_type" required>
                            <option value="">--{{ __('Select Option') }}--</option>
                            <option
                                value="1" {{ (old('download_limit_type') == PLAN_DOWNLOAD_LIMIT_TYPE_UNLIMITED) ? 'selected' : null }}>{{ __('Unlimited') }}</option>
                            <option
                                value="2" {{ (old('download_limit_type') == PLAN_DOWNLOAD_LIMIT_TYPE_LIMITED) ? 'selected' : null }}>{{ __('Limited') }}</option>
                        </select>
                    </div>
                    <div class="col-md-6 durationLimitDiv d-none">
                            <label class="zForm-label" for="name">{{__('Download Limit Per Day')}} <span
                                    class="text-danger">*</span></label>
                            <input type="number" min="1" name="download_limit"
                                   value="{{old('download_limit')}}"
                                   placeholder="{{__('Type Download Limit Per Day')}}"
                                   class="zForm-control download_limit" required>
                    </div>
                    <div class="col-md-6">
                        ...
                        <select class="form-control" name="status" id="status" required>
                            <option value="">--{{ __('Select Option') }}--</option>
                            <option
                                value="1" {{ (old('status') == 1) ? 'selected' : null }}>{{ __('Active') }}</option>
                ...
                <h2 class="fs-24 fw-500 lh-28 text-primary-dark-text pb-24">{{__('Add Benefits')}}</h2>
                <div id="add_repeater" class="">
                    <div data-repeater-list="plan_benefits" class="row rg-24">
                        <div data-repeater-item="" class="col-md-6">
                            <div class="">
                                <label for="point"
                                       class="zForm-label"> {{ __('Point') }} </label>

                                <div class="input-group flex-nowrap">
                                    <input type="text" name="point" id="point" value="" class="zForm-control"
                ...
                <div class="d-flex justify-content-end align-items-center g-10 pt-24 bd-t-one bd-c-stroke mt-24">
